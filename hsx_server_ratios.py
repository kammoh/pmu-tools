...
#
# auto generated TopDown/TMAM 3.4-full description for Intel Xeon E5 v3 (code Named Haswell EP)
...
software prefetches if indeed memory BW limited."""
    ...
    def compute(self, EV):
        try:
            self.val = Load_Miss_Real_Latency(
                self, EV, 4) * EV("L1D_PEND_MISS.REQUEST_FB_FULL:c1", 4) / CLKS(self, EV, 4)
...
subtraction; or multiplication."""
    ...
    def compute(self, EV):
        try:
            self.val = 10 * EV("ARITH.DIVIDER_UOPS", 3) / \
                CORE_CLKS(self, EV, 3)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
