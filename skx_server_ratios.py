...
#
# auto generated TopDown/TMAM 3.4-full description for Intel Xeon Scalable Processors (code named Skylake Server)
...
def Fetched_Uops(self, EV, level):
    EV("IDQ.MS_UOPS", level)
    EV("LSD.UOPS", level)
    EV("IDQ.MITE_UOPS", level)
    EV("IDQ.DSB_UOPS", level)
    return (EV("IDQ.DSB_UOPS", level) + EV("LSD.UOPS", level) + EV("IDQ.MITE_UOPS", level) + EV("IDQ.MS_UOPS", level)) if model in ['KBLR', 'CFL'] else(EV("IDQ.DSB_UOPS", level) + EV("IDQ.MITE_UOPS", level) + EV("IDQ.MS_UOPS", level))
...
def LOAD_L1_MISS_NET(self, EV, level):
    return LOAD_L1_MISS(self, EV, level) + EV("MEM_LOAD_L3_MISS_RETIRED.LOCAL_DRAM", level) + EV("MEM_LOAD_L3_MISS_RETIRED.REMOTE_DRAM", level) + EV("MEM_LOAD_L3_MISS_RETIRED.REMOTE_HITM", level) + EV("MEM_LOAD_L3_MISS_RETIRED.REMOTE_FWD", level)
...
def LOAD_LCL_MEM(self, EV, level):
    return EV("MEM_LOAD_L3_MISS_RETIRED.LOCAL_DRAM", level)*(1 + EV("MEM_LOAD_RETIRED.FB_HIT", level) / LOAD_L1_MISS_NET(self, EV, level))


def LOAD_RMT_MEM(self, EV, level):
    return EV("MEM_LOAD_L3_MISS_RETIRED.REMOTE_DRAM", level)*(1 + EV("MEM_LOAD_RETIRED.FB_HIT", level) / LOAD_L1_MISS_NET(self, EV, level))


def LOAD_RMT_HITM(self, EV, level):
    return EV("MEM_LOAD_L3_MISS_RETIRED.REMOTE_HITM", level)*(1 + EV("MEM_LOAD_RETIRED.FB_HIT", level) / LOAD_L1_MISS_NET(self, EV, level))


def LOAD_RMT_FWD(self, EV, level):
    return EV("MEM_LOAD_L3_MISS_RETIRED.REMOTE_FWD", level)*(1 + EV("MEM_LOAD_RETIRED.FB_HIT", level) / LOAD_L1_MISS_NET(self, EV, level))
...
def LSD_Coverage(self, EV, level):
    EV("LSD.UOPS", level)
    Fetched_Uops(self, EV, level)
    return EV("LSD.UOPS", level) / Fetched_Uops(self, EV, level) if model in ['KBLR', 'CFL'] else 0
...
some period."""
    level = 2
    htoff = False
    sample = ['FRONTEND_RETIRED.LATENCY_GE_16:pp',
              'FRONTEND_RETIRED.LATENCY_GE_32:pp']
    errcount = 0
    sibling = None
    server = True
    ...
    def compute(self, EV):
        try:
            self.val = Pipeline_Width * \
                EV("IDQ_UOPS_NOT_DELIVERED.CYCLES_0_UOPS_DELIV.CORE", 2) / \
                SLOTS(self, EV, 2)
...
through improved hot code layout."""
    ...
    def compute(self, EV):
        try:
            self.val = (EV("ICACHE_16B.IFDATA_STALL", 3) + 2 *
                        EV("ICACHE_16B.IFDATA_STALL:c1:e1", 3)) / CLKS(self, EV, 3)
...
this node may overlap with its siblings."""
    ...
    def compute(self, EV):
        try:
            self.val = (EV("INT_MISC.CLEAR_RESTEER_CYCLES", 3) +
                        BAClear_Cost * EV("BACLEARS.ANY", 3)) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.05) & self.parent.thresh
        except ZeroDivisionError:
...
Misprediction at execution stage."""
    ...
    def compute(self, EV):
        try:
            self.val = Mispred_Clears_Fraction(
                self, EV, 4) * EV("INT_MISC.CLEAR_RESTEER_CYCLES", 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.05) & self.parent.thresh
        except ZeroDivisionError:
...
Clears."""
    ...
    def compute(self, EV):
        try:
            self.val = (1 - Mispred_Clears_Fraction(self, EV, 4)) * \
                EV("INT_MISC.CLEAR_RESTEER_CYCLES", 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.05) & self.parent.thresh
        except ZeroDivisionError:
...
(First fetch or hitting BPU capacity limit)."""
    ...
    def compute(self, EV):
        try:
            self.val = self.Branch_Resteers.compute(
                EV) - EV("INT_MISC.CLEAR_RESTEER_CYCLES", 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.05) & self.parent.thresh
        except ZeroDivisionError:
...
technology/64-ia-32-architectures-optimization-manual.html"""
    ...
    def compute(self, EV):
        try:
            self.val = EV("DSB2MITE_SWITCHES.PENALTY_CYCLES",
                          3) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.05) & self.parent.thresh
        except ZeroDivisionError:
    ...
    def compute(self, EV):
        try:
            self.val = MS_Switches_Cost * \
                EV("IDQ.MS_SWITCHES", 3) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.05) & self.parent.thresh
        except ZeroDivisionError:
...
the Backend (less than four)."""
    ...
    def compute(self, EV):
        try:
            self.val = self.Frontend_Bound.compute(
                EV) - self.Frontend_Latency.compute(EV)
...
technology/64-ia-32-architectures-optimization-manual.html"""
    ...
    def compute(self, EV):
        try:
            self.val = (EV("IDQ.ALL_MITE_CYCLES_ANY_UOPS", 3) -
                        EV("IDQ.ALL_MITE_CYCLES_4_UOPS", 3)) / CORE_CLKS(self, EV, 3)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
categorized here."""
    ...
    def compute(self, EV):
        try:
            self.val = (EV("IDQ.ALL_DSB_CYCLES_ANY_UOPS", 3) -
                        EV("IDQ.ALL_DSB_CYCLES_4_UOPS", 3)) / CORE_CLKS(self, EV, 3)
...
not suit well the LSD structure."""
    ...
    def compute(self, EV):
        try:
            self.val = (EV("LSD.CYCLES_ACTIVE", 3) - EV("LSD.CYCLES_4_UOPS", 3)) / \
                CORE_CLKS(self, EV, 3) if model in ['KBLR', 'CFL'] else 0
            EV("LSD.CYCLES_ACTIVE", 3)
            EV("LSD.CYCLES_4_UOPS", 3)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
Ordering Nukes is another example."""
    ...
    def compute(self, EV):
        try:
            self.val = (EV("UOPS_ISSUED.ANY", 1) - EV("UOPS_RETIRED.RETIRE_SLOTS", 1) +
                        Pipeline_Width * Recovery_Cycles(self, EV, 1)) / SLOTS(self, EV, 1)
...
technology/64-ia-32-architectures-optimization-manual.html"""
    ...
    def compute(self, EV):
        try:
            self.val = Mispred_Clears_Fraction(
                self, EV, 2) * self.Bad_Speculation.compute(EV)
            self.thresh = (self.val > 0.05) & self.parent.thresh
        except ZeroDivisionError:
...
m/d/4/1/d/8/sma.pdf"""
    ...
    def compute(self, EV):
        try:
            self.val = self.Bad_Speculation.compute(
                EV) - self.Branch_Mispredicts.compute(EV)
            self.thresh = (self.val > 0.05) & self.parent.thresh
        except ZeroDivisionError:
...
Bound."""
    ...
    def compute(self, EV):
        try:
            self.val = 1 - (self.Frontend_Bound.compute(EV) +
                            self.Bad_Speculation.compute(EV) + self.Retiring.compute(EV))
            self.thresh = (self.val > 0.1)
        except ZeroDivisionError:
    ...
    def compute(self, EV):
        try:
            self.val = Memory_Bound_Fraction(
                self, EV, 2) * self.Backend_Bound.compute(EV)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
without having that demand load missing the L1 cache."""
    ...
    def compute(self, EV):
        try:
            self.val = (EV("CYCLE_ACTIVITY.STALLS_MEM_ANY", 3) -
                        EV("CYCLE_ACTIVITY.STALLS_L1D_MISS", 3)) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
page walk on an STLB miss.."""
    ...
    def compute(self, EV):
        try:
            self.val = (Mem_STLB_Hit_Cost * EV("DTLB_LOAD_MISSES.STLB_HIT", 4) +
                        EV("DTLB_LOAD_MISSES.WALK_ACTIVE", 4)) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
region than the load is reading."""
    ...
    def compute(self, EV):
        try:
            self.val = 13 * EV("LD_BLOCKS.STORE_FORWARD",
                               4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
L1_Bound regardless of what memory source satisfied them."""
    ...
    def compute(self, EV):
        try:
            self.val = Mem_Lock_St_Fraction(
                self, EV, 4) * ORO_Demand_RFO_C1(self, EV, 4) / CLKS(self, EV, 4)
...
See the Optimization Manual for more details"""
    ...
    def compute(self, EV):
        try:
            self.val = Load_Miss_Real_Latency(
                self, EV, 4) * EV("LD_BLOCKS.NO_SR", 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
...
See the Optimization Manual for more details"""
    ...
    def compute(self, EV):
        try:
            self.val = EV("LD_BLOCKS_PARTIAL.ADDRESS_ALIAS",
                          4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
...
software prefetches if indeed memory BW limited."""
    ...
    def compute(self, EV):
        try:
            self.val = Load_Miss_Real_Latency(
                self, EV, 4) * EV("L1D_PEND_MISS.FB_FULL:c1", 4) / CLKS(self, EV, 4)
...
performance."""
    ...
    def compute(self, EV):
        try:
            self.val = (1 if self.FB_Full.compute(EV) < 1.5 else LOAD_L2_HIT(
                self, EV, 3) / (LOAD_L2_HIT(self, EV, 3) + EV("L1D_PEND_MISS.FB_FULL:c1", 3))) * L2_Bound_Ratio(self, EV, 3)
...
the latency and increase performance."""
    ...
    def compute(self, EV):
        try:
            self.val = (EV("CYCLE_ACTIVITY.STALLS_L2_MISS", 3) -
                        EV("CYCLE_ACTIVITY.STALLS_L3_MISS", 3)) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.05) & self.parent.thresh
        except ZeroDivisionError:
...
false sharing."""
    ...
    sample = ['MEM_LOAD_L3_HIT_RETIRED.XSNP_HIT:pp',
              'MEM_LOAD_L3_HIT_RETIRED.XSNP_MISS:pp']
    errcount = 0
    sibling = None
    server = False
    ...
    def compute(self, EV):
        try:
            self.val = Mem_XSNP_HitM_Cost * \
                (LOAD_XSNP_HITM(self, EV, 4) +
                 LOAD_XSNP_MISS(self, EV, 4)) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
...
sharing can drastically harm multithreaded performance."""
    ...
    def compute(self, EV):
        try:
            self.val = MEM_XSNP_Hit_Cost * \
                LOAD_XSNP_HIT(self, EV, 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
...
its siblings."""
    ...
    def compute(self, EV):
        try:
            self.val = MEM_XSNP_None_Cost * \
                LOAD_L3_HIT(self, EV, 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
Prefetches (also through the compiler).."""
    ...
    def compute(self, EV):
        try:
            self.val = ORO_DRD_Any_Cycles(
                self, EV, 4) / CLKS(self, EV, 4) - self.MEM_Bandwidth.compute(EV)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
to loads from local memory. Caching will improve the latency
and increase performance."""
    ...
    def compute(self, EV):
        try:
            self.val = Mem_Local_DRAM_Cost * \
                LOAD_LCL_MEM(self, EV, 5) / CLKS(self, EV, 5)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
class Remote_DRAM:
    ...
    desc = """
This metric represents how often CPU was likely stalled due
to loads from remote memory. This is caused often due to
non-optimal NUMA allocations."""
    level = 5
    htoff = False
    ...
    def compute(self, EV):
        try:
            self.val = Mem_Remote_DRAM_Cost * \
                LOAD_RMT_MEM(self, EV, 5) / CLKS(self, EV, 5)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
class Remote_Cache:
    ...
    desc = """
This metric represents how often CPU was likely stalled due
to loads from remote cache in other sockets. This is caused
often due to non-optimal NUMA allocations."""
    level = 5
    htoff = False
    sample = ['MEM_LOAD_L3_MISS_RETIRED.REMOTE_HITM:pp',
              'MEM_LOAD_L3_MISS_RETIRED.REMOTE_FWD:pp']
    errcount = 0
    sibling = None
    server = False
    metricgroup = []
    ...
    def compute(self, EV):
        try:
            self.val = (Mem_Remote_HitM_Cost * LOAD_RMT_HITM(self, EV, 5) +
                        Mem_Remote_Fwd_Cost * LOAD_RMT_FWD(self, EV, 5)) / CLKS(self, EV, 5)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
flagged should any of these cases be a bottleneck."""
    ...
    def compute(self, EV):
        try:
            self.val = EV("EXE_ACTIVITY.BOUND_ON_STORES", 3) / \
                CLKS(self, EV, 3)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
...
lines."""
    ...
    def compute(self, EV):
        try:
            self.val = (Mem_Remote_HitM_Cost * EV("OFFCORE_RESPONSE:request=ALL_RFO:response=L3_MISS.REMOTE_HITM", 4) +
                        Mem_XSNP_HitM_Cost * EV("OFFCORE_RESPONSE:request=ALL_RFO:response=L3_HIT.HITM_OTHER_CORE", 4)) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
granularity."""
    ...
    def compute(self, EV):
        try:
            self.val = EV("MEM_INST_RETIRED.SPLIT_STORES", 4) / \
                CORE_CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
...
larger page sizes for large amounts of frequently-used data."""
    ...
    def compute(self, EV):
        try:
            self.val = (Mem_STLB_Hit_Cost * EV("DTLB_STORE_MISSES.STLB_HIT", 4) +
                        EV("DTLB_STORE_MISSES.WALK_ACTIVE", 4)) / CORE_CLKS(self, EV, 4)
            self.thresh = (self.val > 0.05) & self.parent.thresh
        except ZeroDivisionError:
...
analysis as next step."""
    ...
    def compute(self, EV):
        try:
            self.val = self.Backend_Bound.compute(
                EV) - self.Memory_Bound.compute(EV)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
same uop."""
    ...
    def compute(self, EV):
        try:
            self.val = (Backend_Bound_Cycles(self, EV, 3) - EV("CYCLE_ACTIVITY.STALLS_MEM_ANY", 3) - EV("EXE_ACTIVITY.BOUND_ON_STORES", 3)) / CLKS(self, EV, 3) if(EV("ARITH.DIVIDER_ACTIVE", 3) < EV("EXE_ACTIVITY.EXE_BOUND_0_PORTS", 3)
                                                                                                                                                                   )else(Backend_Bound_Cycles(self, EV, 3) - EV("CYCLE_ACTIVITY.STALLS_MEM_ANY", 3) - EV("EXE_ACTIVITY.BOUND_ON_STORES", 3) - EV("EXE_ACTIVITY.EXE_BOUND_0_PORTS", 3)) / CLKS(self, EV, 3)
...
technology/64-ia-32-architectures-optimization-manual.html"""
    ...
    def compute(self, EV):
        try:
            self.val = Cycles_0_Ports_Utilized(
                self, EV, 4) / CORE_CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
...
of-order execution which may limit performance."""
    ...
    def compute(self, EV):
        try:
            self.val = EV("PARTIAL_RAT_STALLS.SCOREBOARD", 5) / \
                CLKS(self, EV, 5)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
as next step."""
    ...
    def compute(self, EV):
        try:
            self.val = Cycles_1_Port_Utilized(
                self, EV, 4) / CORE_CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
...
and 2nd branch)"""
    ...
    def compute(self, EV):
        try:
            self.val = EV("UOPS_DISPATCHED_PORT.PORT_0", 5) / \
                CORE_CLKS(self, EV, 5)
...
dispatched uops on execution port 1 (ALU)"""
    ...
    def compute(self, EV):
        try:
            self.val = EV("UOPS_DISPATCHED_PORT.PORT_1", 5) / \
                CORE_CLKS(self, EV, 5)
            self.thresh = (self.val > 0.6)
        except ZeroDivisionError:
...
class Port_2:
    ...
    desc = """
This metric represents Core fraction of cycles CPU
dispatched uops on execution port 2 (Loads and Store-
address)"""
    level = 5
    htoff = False
    ...
    def compute(self, EV):
        try:
            self.val = EV("UOPS_DISPATCHED_PORT.PORT_2", 5) / \
                CORE_CLKS(self, EV, 5)
            self.thresh = (self.val > 0.6)
        except ZeroDivisionError:
...
class Port_3:
    ...
    desc = """
This metric represents Core fraction of cycles CPU
dispatched uops on execution port 3 (Loads and Store-
address)"""
    level = 5
    htoff = False
    ...
    def compute(self, EV):
        try:
            self.val = EV("UOPS_DISPATCHED_PORT.PORT_3", 5) / \
                CORE_CLKS(self, EV, 5)
            self.thresh = (self.val > 0.6)
        except ZeroDivisionError:
...
dispatched uops on execution port 4 (Store-data)"""
    ...
    def compute(self, EV):
        try:
            self.val = EV("UOPS_DISPATCHED_PORT.PORT_4", 5) / \
                CORE_CLKS(self, EV, 5)
            self.thresh = (self.val > 0.6)
        except ZeroDivisionError:
...
ALU; [HSW+] ALU)"""
    ...
    def compute(self, EV):
        try:
            self.val = EV("UOPS_DISPATCHED_PORT.PORT_5", 5) / \
                CORE_CLKS(self, EV, 5)
            self.thresh = (self.val > 0.6)
        except ZeroDivisionError:
...
simple ALU)"""
    ...
    def compute(self, EV):
        try:
            self.val = EV("UOPS_DISPATCHED_PORT.PORT_6", 5) / \
                CORE_CLKS(self, EV, 5)
            self.thresh = (self.val > 0.6)
        except ZeroDivisionError:
...
dispatched uops on execution port 7 (simple Store-address)"""
    ...
    def compute(self, EV):
        try:
            self.val = EV("UOPS_DISPATCHED_PORT.PORT_7", 5) / \
                CORE_CLKS(self, EV, 5)
            self.thresh = (self.val > 0.6)
        except ZeroDivisionError:
...
such as vectorization."""
    ...
    def compute(self, EV):
        try:
            self.val = self.Retiring.compute(
                EV) - self.Microcode_Sequencer.compute(EV)
...
(FP) uops fraction the CPU has executed (retired)"""
    ...
    def compute(self, EV):
        try:
            self.val = self.X87_Use.compute(
                EV) + self.FP_Scalar.compute(EV) + self.FP_Vector.compute(EV)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
...
vectors."""
    ...
    def compute(self, EV):
        try:
            self.val = EV("UOPS_EXECUTED.X87", 4) / \
                EV("UOPS_EXECUTED.THREAD", 4)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
what limits (compiler) generation of vector code."""
    ...
    def compute(self, EV):
        try:
            self.val = (EV("FP_ARITH_INST_RETIRED.SCALAR_SINGLE", 4) + EV(
                "FP_ARITH_INST_RETIRED.SCALAR_DOUBLE", 4)) / EV("UOPS_RETIRED.RETIRE_SLOTS", 4)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
across all vector widths.. Check if vector width is expected"""
    ...
    def compute(self, EV):
        try:
            self.val = (EV("FP_ARITH_INST_RETIRED.128B_PACKED_DOUBLE", 4) + EV("FP_ARITH_INST_RETIRED.128B_PACKED_SINGLE", 4) + EV("FP_ARITH_INST_RETIRED.256B_PACKED_DOUBLE", 4) + EV(
                "FP_ARITH_INST_RETIRED.256B_PACKED_SINGLE", 4) + EV("FP_ARITH_INST_RETIRED.512B_PACKED_DOUBLE", 4) + EV("FP_ARITH_INST_RETIRED.512B_PACKED_SINGLE", 4)) / EV("UOPS_RETIRED.RETIRE_SLOTS", 4)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
...
avoided.."""
    ...
    def compute(self, EV):
        try:
            self.val = Retire_Uop_Fraction(
                self, EV, 2) * EV("IDQ.MS_UOPS", 2) / SLOTS(self, EV, 2)
...
cases."""
    ...
    def compute(self, EV):
        try:
            self.val = Avg_Assist_Cost * \
                (EV("FP_ASSIST.ANY", 3) + EV("OTHER_ASSISTS.ANY", 3)) / \
                SLOTS(self, EV, 3)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
per non-speculative branch misprediction (jeclear)"""
    ...
    def compute(self, EV):
        try:
            ...
            handle_error_metric(
                self, "Branch_Misprediction_Cost zero division")
...
codes."""
    ...
    def compute(self, EV):
        try:
            ...
            handle_error_metric(
                self, "Power_License0_Utilization zero division")
...
512-bit instructions."""
    ...
    def compute(self, EV):
        try:
            ...
            handle_error_metric(
                self, "Power_License1_Utilization zero division")
...
This includes high current AVX 512-bit instructions."""
    ...
    def compute(self, EV):
        try:
            ...
            handle_error_metric(
                self, "Power_License2_Utilization zero division")
...
class Setup:
    def __init__(self, r):
        ...
        r.run(n)
        o["Branch_Resteers"] = n
        n = Mispredicts_Resteers()
        r.run(n)
        o["Mispredicts_Resteers"] = n
        n = Clears_Resteers()
        r.run(n)
        o["Clears_Resteers"] = n
        n = Unknown_Branches()
        r.run(n)
        o["Unknown_Branches"] = n
        n = DSB_Switches()
        r.run(n)
        o["DSB_Switches"] = n
        n = LCP()
        ...
        r.run(n)
        o["MS_Switches"] = n
        n = Frontend_Bandwidth()
        r.run(n)
        o["Frontend_Bandwidth"] = n
        n = MITE()
        r.run(n)
        o["MITE"] = n
        n = DSB()
        ...
        r.run(n)
        o["Bad_Speculation"] = n
        n = Branch_Mispredicts()
        r.run(n)
        o["Branch_Mispredicts"] = n
        n = Machine_Clears()
        r.run(n)
        o["Machine_Clears"] = n
        n = Backend_Bound()
        ...
        r.run(n)
        o["L3_Bound"] = n
        n = Contested_Accesses()
        r.run(n)
        o["Contested_Accesses"] = n
        n = Data_Sharing()
        r.run(n)
        o["Data_Sharing"] = n
        n = L3_Hit_Latency()
        ...
        n = Ports_Utilization()
        r.run(n)
        o["Ports_Utilization"] = n
        n = G0_Ports_Utilized()
        r.run(n)
        o["G0_Ports_Utilized"] = n
        n = Serializing_Operation()
        r.run(n)
        o["Serializing_Operation"] = n
        n = G1_Port_Utilized()
        r.run(n)
        o["G1_Port_Utilized"] = n
        n = G2_Ports_Utilized()
        r.run(n)
        o["G2_Ports_Utilized"] = n
        n = G3m_Ports_Utilized()
        r.run(n)
        o["G3m_Ports_Utilized"] = n
        n = Port_0()
        r.run(n)
        o["Port_0"] = n
        n = Port_1()
        ...
        r.run(n)
        o["Other"] = n
        n = Microcode_Sequencer()
        r.run(n)
        o["Microcode_Sequencer"] = n
        n = Assists()
        r.run(n)
        o["Assists"] = n
        ...
        r.metric(n)
        o["BPTB"] = n
        n = Metric_IFetch_Line_Utilization()
        r.metric(n)
        o["IFetch_Line_Utilization"] = n
        n = Metric_DSB_Coverage()
        r.metric(n)
        o["DSB_Coverage"] = n
        n = Metric_LSD_Coverage()
        r.metric(n)
        o["LSD_Coverage"] = n
        n = Metric_CPI()
        r.metric(n)
        ...
        r.metric(n)
        o["IpCall"] = n
        n = Metric_IpArith_Scalar_SP()
        r.metric(n)
        o["IpArith_Scalar_SP"] = n
        n = Metric_IpArith_Scalar_DP()
        r.metric(n)
        o["IpArith_Scalar_DP"] = n
        n = Metric_IpArith_AVX128()
        r.metric(n)
        o["IpArith_AVX128"] = n
        n = Metric_IpArith_AVX256()
        r.metric(n)
        o["IpArith_AVX256"] = n
        n = Metric_IpArith_AVX512()
        r.metric(n)
        o["IpArith_AVX512"] = n
        n = Metric_Instructions()
        r.metric(n)
        o["Instructions"] = n
        n = Metric_CoreIPC()
        r.metric(n)
        ...
        r.metric(n)
        o["FLOPc"] = n
        n = Metric_FP_Arith_Utilization()
        r.metric(n)
        o["FP_Arith_Utilization"] = n
        n = Metric_ILP()
        r.metric(n)
        o["ILP"] = n
        n = Metric_Branch_Misprediction_Cost()
        r.metric(n)
        o["Branch_Misprediction_Cost"] = n
        n = Metric_IpMispredict()
        r.metric(n)
        o["IpMispredict"] = n
        n = Metric_CORE_CLKS()
        r.metric(n)
        o["CORE_CLKS"] = n
        n = Metric_Load_Miss_Real_Latency()
        r.metric(n)
        o["Load_Miss_Real_Latency"] = n
        n = Metric_MLP()
        r.metric(n)
        o["MLP"] = n
        n = Metric_Page_Walks_Utilization()
        r.metric(n)
        o["Page_Walks_Utilization"] = n
        n = Metric_L1D_Cache_Fill_BW()
        r.metric(n)
        o["L1D_Cache_Fill_BW"] = n
        n = Metric_L2_Cache_Fill_BW()
        r.metric(n)
        o["L2_Cache_Fill_BW"] = n
        n = Metric_L1MPKI()
        r.metric(n)
        o["L1MPKI"] = n
        n = Metric_L2MPKI()
        ...
        r.metric(n)
        o["L3MPKI"] = n
        n = Metric_CPU_Utilization()
        r.metric(n)
        o["CPU_Utilization"] = n
        n = Metric_Average_Frequency()
        r.metric(n)
        o["Average_Frequency"] = n
        n = Metric_GFLOPs()
        r.metric(n)
        o["GFLOPs"] = n
        n = Metric_Turbo_Utilization()
        r.metric(n)
        o["Turbo_Utilization"] = n
        n = Metric_Power_License0_Utilization()
        r.metric(n)
        o["Power_License0_Utilization"] = n
        n = Metric_Power_License1_Utilization()
        r.metric(n)
        o["Power_License1_Utilization"] = n
        n = Metric_Power_License2_Utilization()
        r.metric(n)
        o["Power_License2_Utilization"] = n
        n = Metric_SMT_2T_Utilization()
        r.metric(n)
        o["SMT_2T_Utilization"] = n
        n = Metric_Kernel_Utilization()
        r.metric(n)
        o["Kernel_Utilization"] = n
        n = Metric_DRAM_BW_Use()
        r.metric(n)
        o["DRAM_BW_Use"] = n
        n = Metric_MEM_Request_Latency()
        r.metric(n)
        o["MEM_Request_Latency"] = n
        n = Metric_MEM_Parallel_Requests()
        r.metric(n)
        o["MEM_Parallel_Requests"] = n
        n = Metric_MEM_DRAM_Read_Latency()
        r.metric(n)
        o["MEM_DRAM_Read_Latency"] = n
        n = Metric_Time()
        r.metric(n)
        o["Time"] = n
        n = Metric_Socket_CLKS()
        r.metric(n)
        o["Socket_CLKS"] = n
        ...
        o["Mispredicts_Resteers"].sibling = (
            o["Clears_Resteers"], o["Branch_Mispredicts"],)
        o["Clears_Resteers"].sibling = (
            o["Mispredicts_Resteers"], o["Branch_Mispredicts"],)
        o["MS_Switches"].sibling = (
            o["Serializing_Operation"], o["Microcode_Sequencer"],)
        o["Branch_Mispredicts"].sibling = (
            o["Mispredicts_Resteers"], o["Clears_Resteers"],)
        o["L1_Bound"].sibling = (o["G1_Port_Utilized"],)
        o["Lock_Latency"].sibling = (o["Store_Latency"],)
        o["FB_Full"].sibling = (
            o["SQ_Full"], o["MEM_Bandwidth"], o["Store_Latency"],)
