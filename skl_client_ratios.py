...
#
# auto generated TopDown/TMAM 3.4-full description for Intel 6th/7th gen Core (code named Skykale/Kabylake)
...
lines."""
    ...
    def compute(self, EV):
        try:
            self.val = Mem_XSNP_HitM_Cost * \
                EV("OFFCORE_RESPONSE:request=ALL_RFO:response=L3_HIT.SNOOP_HITM",
                   4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
class Setup:
    def __init__(self, r):
        ...
        n = Metric_MEM_Read_Latency()
        r.metric(n)
        o["MEM_Read_Latency"] = n
        n = Metric_MEM_Parallel_Reads()
        r.metric(n)
        o["MEM_Parallel_Reads"] = n
