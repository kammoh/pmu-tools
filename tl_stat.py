    ...
    def referenced_check(self, res):
        ...
        if len(list(res.keys())) > 0:
            ...
            if len(referenced) != len(r) and not self.quiet:
                print("warning: %d results not referenced:" %
                      (len(r) - len(referenced)), end=' ', file=sys.stderr)
                print(" ".join(["%d" % x for x in sorted(
                    set(range(len(r))) - referenced)]), file=sys.stderr)

    def compute_errors(self):
        if self.errcount > 0 and self.errors != self.prev_errors:
            if not self.quiet:
                print("warning: %d division by zero errors:" %
                      (self.errcount), end=' ', file=sys.stderr)
