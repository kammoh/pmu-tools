...
categorized under the Bad Speculation category"""
    ...
    def compute(self, EV):
        try:
            self.val = (EV("UOPS_ISSUED.ANY", 1) - EV("UOPS_RETIRED.RETIRE_SLOTS",
                                                      1) + PipelineWidth * EV("INT_MISC.RECOVERY_CYCLES", 1)) / SLOTS(EV)
...
of required resources for accepting more uops in the Backend of the pipeline."""
    ...
    def compute(self, EV):
        try:
            self.val = 1 - (self.FrontendBound.compute(EV) +
                            self.BadSpeculation.compute(EV) + self.Retiring.compute(EV))
            self.thresh = self.val > 0.2
        except ZeroDivisionError:
            self.val = 0
            self.thresh = False
